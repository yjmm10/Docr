...
class CRNN(CVModel):
    ...
    def batch_inference(self, images):
        ...
        for beg_img_no in range(0, img_num, self.rec_batch_num):
            ...
            max_wh_ratio = max(max([images[i].shape[1]*1.0 / images[i].shape[0] \
                                for i in indices[beg_img_no:end_img_no]]), max_wh_ratio)

            for ino in range(beg_img_no, end_img_no):
                norm_img = self.pre_process(images[indices[ino]],max_wh_ratio=max_wh_ratio)
...
class CTCLabelDecode(object):
    ...
    def __init__(self, char_list:List=None, use_space_char=False,
                 **kwargs):
